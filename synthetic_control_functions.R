...
rrPredQuantiles <- function(impact, denom_data = NULL, mean, sd, eval_period, post_period, trend = FALSE) {
  ...
  roll_rr <- t(apply(roll_rr_est, 1, quantile, probs = c(0.025, 0.5, 0.975), na.rm = TRUE))
	...
	#Outputs pointwise Rate ratios, SD, and covariance matrices, which can be used in second stage meta analysis
	if (offset) {
	  obs_t<-exp(all_cause_data) * exp(impact$series$response * sd + mean)
	} else {
	  obs_t<-  exp(impact$series$response * sd + mean)
	} 
